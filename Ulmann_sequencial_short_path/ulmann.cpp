...
void degree_sort(vector<vector<int> > &A,vector<vector<int> > &M, bool row)
{
    ...
    for (int i = 0; i < nei_C; i++)
    {
        ...
        //maxIndex = std::distance(nei_num, std::max_element(nei_num + i, nei_num + nei_C));
...
// prune all elements in M that the dikstra distance in A is bigger than B
                ...
                vector<vector<int> > &A_dist, vector<vector<int> > &B_dist, int A_index, int B_index)
{
    ...
    for (int i = 0; i < A_R; i++)
    {
        ...
        if (i != A_index)
        {
            ...
            for (int j = 0; j < B_R; j++)
            {
                if (A_dist[A_index][i] < B_dist[B_index][j]) // if the distance to current point is smaller than the corresponding distance in B, cut the mapping
                    M[i][j] = 0;
            }
        }
